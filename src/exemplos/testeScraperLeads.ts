...
async function testeScraperLeads() {
  try {
    // Inicializa o extrator
    const extrator = MapsScraper.getInstance();

    // Testa apenas o nicho de Transportadoras
    const nicho = { termo: 'Transportadoras São Paulo', descricao: 'Transportadoras' };
    
    logger.info(`\n🚀 Testando extração de leads para: ${nicho.descricao}`);
    logger.info(`🔍 Termo de busca: ${nicho.termo}`);
    logger.info('⏳ Iniciando processo de extração...');
    
    // Busca estabelecimentos (limitado a 2 para teste)
    const leads = await extrator.buscarEstabelecimentos(
      nicho.termo,
      2 // Limitado a 2 resultados para teste
    );
...
⭐ Avaliação: ${lead.avaliacao.toFixed(1)} (${lead.totalAvaliacoes} avaliações)
    ...
    // Fecha o navegador
    await extrator.closeBrowser();

    logger.info('\n✅ Teste de extração de leads concluído com sucesso!');

  } catch (error) {
    logger.error('❌ Erro ao executar teste de extração de leads:', error);
  }
}
